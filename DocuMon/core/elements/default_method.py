...
class MethodD(AbsDescriptor):
    args: List[Argument]
    add_args: Optional[Argument]
    kwargs: Optional[Argument]
    returns: typing.Union[Returns, List[Returns]]

    def __init__(self, extractor, element: callable, _class):
        self.args = []
        self.add_args = None
        self.kwargs = None
        super().__init__(
            extractor,
            *[q for q in [d.__dict__.get(element.__name__) for d in _class.__mro__] if q],
            _type="method"
        )
    ...
    @property
    def class_name(self):
        return ".".join(self.element.__qualname__.split(".")[:-1])